...
try {
    ...
    if (!empty($search)) {
        $where_conditions[] = "(l.nama_kategori LIKE ? OR l.deskripsi LIKE ?)";
        $search_param = "%$search%";
        $params[] = $search_param;
        $params[] = $search_param;
    }
    ...
    $count_query = "SELECT COUNT(*) as total FROM kategori l $where_clause";
    ...
    FROM kategori l
    LEFT JOIN barang b ON l.id = b.kategori_id
    $where_clause
    GROUP BY l.id, l.nama_kategori, l.deskripsi, l.created_at, l.updated_at
    ORDER BY l.nama_kategori ASC
